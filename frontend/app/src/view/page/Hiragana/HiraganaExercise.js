...
import { loadQuestionHiraganaData, suggestAnswer } from '../../../service/process/loadData'
import { 
        loadQuestion, setReload, setTypeQuestion, checkAnswer, 
        setCorrectNoti, setIncorrectNoti, setSuggestedData, setIsSuggest
      } from '../../../service/reducer/ExerciseReducer'
...
const HiraganaExercise = () => {
  ...
  useEffect(() => {
    const loadQuestionEvent = async() => {
      if(reload) {
        const data = await loadQuestionHiraganaData(typeQuestion)
